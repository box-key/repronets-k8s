...
def create_app():
    ...
    mlflow.pyfunc.load_model(f"models:/{model_name}/{model_version}", dst_path='.')
    logger.info(f"Download model artifacts")
    net_model = ctranslate2.Translator("artifacts/ctranslate2_released")
    logger.info(f"Load {model_name=}, {model_version=}")
    # init flask objects
    app = Flask(__name__)
    api = Api(app)
